...
def render_health() -> str:
    ...
    for unit in health.get("unit", []):
        lines.append(f"| `{unit['id']}` | {unit['kind']} |")
    lines.extend(
        [
            "",
            "## Packages",
    ...
    for package in health.get("package", []):
    ...
    for portal in health.get("portal", []):
        lines.append(f"| `{portal['id']}` |")
    lines.extend(
        [
            "",
            "## Extra commands",
    ...
    for command in health.get("command", []):
        lines.append(f"| `{command['id']}` |")
    lines.append("")
    return "\n".join(lines)


def render_health_checklist() -> str:
    health = load("health.toml")
    lines = []
    for unit in health.get("unit", []):
        lines.append(f"unit\t{unit['kind']}\t{unit['id']}")
    for package in health.get("package", []):
        lines.append(f"package\t{package['id']}")
    for portal in health.get("portal", []):
        lines.append(f"portal\t{portal['id']}")
    for command in health.get("command", []):
        lines.append(f"command\t{command['id']}")
    lines.append("")
    return "\n".join(lines)
...
def generated_documents(data: dict[str, list[dict]]) -> dict[Path, str]:
    return {
        ...
        ROOT / "docs" / "HEALTH.md": render_health(),
        ROOT / "docs" / "health-checklist.txt": render_health_checklist(),
    }
