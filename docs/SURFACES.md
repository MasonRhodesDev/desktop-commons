...
| span-lines-v0 | provisional | OpenTelemetry's data model, not its wire protocol. Read a record by splitting on whitespace, then each token on its first `=`, then percent-decoding the value; that rule is exact because keys and event names are `&'static str` in the API and values are percent-encoded, so no field can contain whitespace or `=`. A span carries `t_us` (start) and `dur_us`, an event carries `t_us`, and `seq` totally orders one process's records - so a span is an interval and overlap is decidable, which record order alone cannot give since spans emit at their end. `t_us` is per-process; comparing across processes is journald's `__MONOTONIC_TIMESTAMP` (`-o short-precise`). Wall-clock stamps are deliberately absent - journald supplies them along with `_BOOT_ID` and `_PID`, and duplicating them invites disagreement. Note journald attributes an entry by the writer's cgroup rather than the unit owning the stream, so a producer launched in a transient scope is attributed to that scope (`_SYSTEMD_USER_UNIT=run-pNNN-iNNN.scope`), whose name differs every run; readers filter by `_COMM`, never by `-u`. Records are capped at 2048 bytes and written in one `write_all`, which narrows but does not close the window in which a foreign writer on the same fd can interleave - POSIX guarantees atomicity only up to `PIPE_BUF` and only for pipes, while journald's stdout stream is an `AF_UNIX SOCK_STREAM`. Over the cap a record is truncated and flagged `trunc=1`. Detail is off|session|frames; anything unrecognised, including unset, means session, so a typo cannot silence a consumer. Producers propagate W3C `TRACEPARENT`, carrying inbound trace flags verbatim without letting them gate local emission. Two producer APIs write this format and are indistinguishable to a reader: the crate's own dependency-free `Trace`/`Span`, and - behind the `tracing` feature - a `tracing_subscriber` layer taking the standard macros, where INFO and above is the session tier and DEBUG/TRACE the frames tier. The layer admits only an explicit allowlist of target prefixes, because installing a subscriber otherwise collects every instrumented crate in the process, and on a login screen that is a disclosure question rather than a volume one. Output goes to a pluggable `Sink` (stderr by default), and `span_lines::exit` closes still-open spans with `status=exit` before terminating, since `std::process::exit` runs no destructors. |
