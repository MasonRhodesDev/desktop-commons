...
| P3 | session-config | composition | owned | hypr-de | — | low | hypr-DE is packaged Hyprland configuration and the runtime packages it needs, not a greeter session. Log into stock Hyprland (uwsm). Dotfiles are a personal overlay, not a competing distribution. Fresh-install validated 2026-08-23 at 0.2.21 on both Arch (57/57) and Fedora 44 (56/56, first Fedora run) via tests/qemu-fresh. |
...
| P2 | screen-recording | desktop-integration | hybrid | hypr-de | gpu-screen-recorder, PipeWire | medium | hypr-DE owns recording workflow and Waybar integration; gpu-screen-recorder is the selected capture engine but remains a Fedora packaging gate. Recorder state moved out of predictable /tmp paths into the per-user runtime dir, and the stop path confirms the pid is still the recorder before signalling it. |
| P1 | screen-sharing | desktop-integration | external | — | PipeWire, xdg-desktop-portal-hyprland | high | External portals provide capture consent and stream negotiation; no suite-level doctor currently verifies the path. hypr-DE's picker wrapper deduplicates the several portal sessions one share spawns, and that replay is scoped to the requesting client (read from the portal frontend's session object paths) so a second consumer cannot inherit consent (surface screencast-consent-scope-v1). |
...
| P1 | screen-lock | login | hybrid | vigil | ext-session-lock-v1, PAM, systemd-logind | high | Vigil owns login and lock. hypr-DE invokes vigil-lock and lists vigil as a runtime dependency. Crash recovery still lacks a service owner, but launch failure now has one: hypr-DE's lock_cmd retries unscoped, tries other installed lockers, and terminates the session rather than returning an unlocked desktop, with the failsafe settable only from a root-owned policy file (ADR 0005). |
...
| P2 | configuration-deployment | operations | hybrid | hypr-de | chezmoi | medium | hypr-DE owns system defaults while chezmoi owns personal overrides and machine variance. Upgrade path is covered by tests/qemu-fresh/run.sh upgrade (rolls back one release, autologins, upgrades under a live session); hypr-de ships a PostTransaction hook that reloads live Hyprland sessions so replacing a watched config file cannot latch a config-error banner. |
...
| P1 | package-user-overlay-shadowing | operations | gap | — | systemd user units, chezmoi | high | Legacy dotfile units and documentation can shadow package-owned gaming/session services or reference payloads no longer present. The security case of this class is closed for hypr-DE units: hypridle was resolved through $PATH, so a file named hypridle in ~/.local/bin durably disabled auto-lock while the unit still reported active. Exec lines now name absolute paths and check-units.sh rejects any unit that resolves an executable through PATH (assertion idle-driver-is-not-path-resolved). |
...
| P1 | suspend-policy | power | hybrid | hyprstate | systemd-logind, systemd suspend targets | high | The decided ladder (hyprstate POWER_SPEC.md, 2026-09-04): keep-awake claims govern only the unlocked machine; once locked, blank and suspend march unconditionally. hyprstate owns lid/idle grace, lock-before-suspend, cancellation, and the battery-low override; hypr-DE's hypridle listener may only request via the suspend-request file; logind performs suspension. |
